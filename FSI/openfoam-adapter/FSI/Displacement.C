...
void preciceAdapter::FSI::Displacement::write(double * buffer, bool meshConnectivity, const unsigned int dim)
{
	...
	// write the displacements to precice buffer
        forAll(pointDisplacement_->boundaryFieldRef()[patchID], i)
        {
            buffer[bufferIndex++] = pointDisplacementFluidPatch[i][0];
            buffer[bufferIndex++] = pointDisplacementFluidPatch[i][1];
            if(dim ==3)
                buffer[bufferIndex++] = pointDisplacementFluidPatch[i][2];
        }
    }
}
