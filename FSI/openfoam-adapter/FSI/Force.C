...
:
  ...
  nameForce_(nameForce)
{
    //What about type "basic"?
    if (solverType_.compare("incompressible") != 0 && solverType_.compare("compressible") != 0 && solverType_.compare("basic") != 0) 
    {
...
void preciceAdapter::FSI::Force::write(double * buffer, bool meshConnectivity, const unsigned int dim)
{
    ...
    for (uint j = 0; j < patchIDs_.size(); j++)
    {
        ...
        // Pressure forces
        if (solverType_.compare("incompressible") == 0 || solverType_.compare("basic") == 0)
        {
        ...
        {
            Force_->boundaryFieldRef()[patchID] =
	      Sfb[patchID] * (pb[patchID] - 1.0e5);//pb[patchID];
        }
        else
        {
            FatalErrorInFunction
                << "Forces calculation does only support "
                << "compressible or incompressible solver type._"
                << exit(FatalError);
        }
